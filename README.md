...
Customers can reschedule any **pending** booking before the guide accepts it.
...
Rules:
- Only works while booking status is `pending`
- New slot must belong to the same guide
- Standard double-booking check still applies
- The old slot is freed back to `available` automatically
- Guide receives an updated email notification marked `[RESCHEDULED]`

**UI:** A "Reschedule" button appears next to "Cancel Request" on every pending booking card in the User Dashboard. Clicking it opens a modal showing all the guide's currently available slots.

---

## 🐳 Full Docker Deployment

All five services (MySQL, MongoDB, Redis, backend, frontend) run via Docker Compose.

```bash
# 1. Configure environment
cp backend/.env.example backend/.env
# Edit backend/.env — set JWT_SECRET, Cloudinary, Gmail, GitHub OAuth keys

# 2. Build and start everything
docker-compose up --build -d

# 3. Seed the database (run once after first start)
docker exec yatrika_backend python scripts/seed_places.py
...
**Architecture:**
- `frontend` container (nginx) serves the React build and proxies `/api/*` to `backend`
- `backend` container runs FastAPI with Uvicorn
- Database containers are health-checked; backend waits for them before starting

**Local development** (without Docker for app services) still works the same way via `npm run dev` and `uvicorn main:app --reload`.
