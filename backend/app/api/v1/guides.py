...
from app.utils.notification_helpers import create_notification, notify_admins
...
def get_my_guide_profile(
    ...
    # Only return ACTIVE assigned places (not deleted ones)
    places = (
...
def get_my_slots(
    ...
    gp = db.query(GuideProfile).filter(GuideProfile.user_id == current_user.id).first()
    # Only return slots for places that are still active
    slots = (
        db.query(GuideTimeSlot)
        .join(TouristPlace, TouristPlace.id == GuideTimeSlot.place_id)
        .filter(
            GuideTimeSlot.guide_id == gp.id,
            TouristPlace.is_active == True,
        )
        .order_by(GuideTimeSlot.slot_date, GuideTimeSlot.start_time)
        .all()
    )
    return [
        {
...
def create_slot(
    ...
    # Check guide is assigned to this place AND the place is still active
    assigned = db.query(GuidePlaceAssignment).join(
        TouristPlace, TouristPlace.id == GuidePlaceAssignment.place_id
    ).filter(
        GuidePlaceAssignment.guide_id == gp.id,
    ...
    if not assigned:
        raise HTTPException(403, "You are not assigned to this place or it has been removed")
...
def respond_to_booking(
    ...
    if payload.action == "accept":
        ...
        create_notification(
            ...
            "Booking accepted",
            f"{current_user.full_name} accepted your tour request at {place.name if place else 'the place'} on {booking.booking_date}.",
            "/user/dashboard",
        )
        db.commit()
        ...
        create_notification(
            ...
            "Booking declined",
            f"{current_user.full_name} declined your tour request at {place.name if place else 'the place'} on {booking.booking_date}.",
            "/user/dashboard",
        )
        db.commit()
        background_tasks.add_task(
...
def submit_place_request(
    ...
    notify_admins(
        ...
        "New place request",
        f"{current_user.full_name} submitted a new place: {payload.place_name} ({payload.city}).",
        "/admin/approvals",
    )
    db.commit()
...
def request_place_assignment(
    ...
    notify_admins(
        ...
        f"{current_user.full_name} requested to guide {place.name} ({place.city}).",
        "/admin/approvals",
    )
    db.commit()
...
def get_guide_slots(
    ...
    # Default: never return past slots
    effective_from = from_date if from_date and from_date >= today else today
    query = db.query(GuideTimeSlot).filter(
        GuideTimeSlot.guide_id == guide_id,
        GuideTimeSlot.slot_date >= effective_from,
    )
    if to_date:
        query = query.filter(GuideTimeSlot.slot_date <= to_date)
