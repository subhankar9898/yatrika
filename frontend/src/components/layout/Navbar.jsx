            ...
            >
                <div className="min-h-full flex items-start justify-center p-4 pt-16 sm:pt-20 pb-16">
                    <motion.div 
                        ...
                        className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border border-white/50 w-full max-w-md p-5 sm:p-6" 
                    ...
                    <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" className="input text-sm bg-gray-50 cursor-not-allowed capitalize" value={user?.role || ''} disabled />
                        </div>
...
function NotificationPanel({ onClose }) {
    ...
    return (
        <motion.div 
            ...
            className="absolute right-0 top-12 w-80 bg-white/90 backdrop-blur-lg rounded-xl shadow-2xl border border-white/50 z-50 overflow-hidden"
        >
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100/50">
                <span className="font-semibold text-gray-800 text-sm">Notifications</span>
                <div className="flex items-center gap-2">
                    {data?.unread_count > 0 && (
                        <button onClick={() => markAll.mutate()} className="text-xs text-emerald-600 hover:underline flex items-center gap-1">
                            <FiCheckCircle size={12} /> Mark all read
                        </button>
                    )}
                    {notifs.length > 0 && (
                        <button onClick={() => { if(window.confirm('Clear all notifications?')) clearAll.mutate() }} className="text-xs text-gray-500 hover:text-red-500 transition-colors flex items-center gap-1">
                            Clear
                        </button>
                    )}
            ...
            <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
                {notifs.length === 0 ? (
                    ...
                    <div
                        key={n.id}
                        className={`px-4 py-3 flex gap-3 hover:bg-gray-50 cursor-pointer transition-colors ${!n.is_read ? 'bg-emerald-50' : ''}`}
                    ...
                    >
                        ...
                        {!n.is_read && <div className="w-2 h-2 rounded-full bg-emerald-500 mt-1 flex-shrink-0" />}
                    </div>
                ))}
            </div>
        </motion.div>
        ...
        <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    <Link to="/" className="flex items-center gap-2 group animate-fade-in">
                        ...
                        <span className="text-xl font-bold text-emerald-600 group-hover:text-emerald-700 transition-colors">Yatrika</span>
                    </Link>

                    {/* Desktop */}
                    <div className="hidden md:flex items-center gap-5">
                        {accessToken && (
                            <Link to="/places" className={`text-sm font-medium transition-colors hover:text-emerald-600 ${isActive('/places') ? 'text-emerald-600' : 'text-gray-600'}`}>
                                Explore Places
                            </Link>
                        )}
                        {accessToken ? (
                            <div className="flex items-center gap-3">
                                <Link to={dashboardPath} className={`text-sm font-medium transition-colors hover:text-emerald-600 ${isActive(dashboardPath) ? 'text-emerald-600' : 'text-gray-600'}`}>
                                    Dashboard
                                </Link>

                                {/* Bell */}
                                <div className="relative" ref={bellRef}>
                                    <button onClick={() => setBellOpen(v => !v)} className="relative p-2 rounded-full hover:bg-slate-100 transition-colors" aria-label="Notifications">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-full px-3 py-1.5 hover:border-emerald-400 hover:bg-emerald-50/40 transition-all cursor-pointer group"
                                    title="Edit my profile"
                                >
                                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-sm overflow-hidden border border-emerald-200">
                                    ...
                                    <FiEdit2 size={12} className="text-gray-400 group-hover:text-emerald-600 transition-colors" />
                                </button>
            ...
            {menuOpen && (
                <motion.div 
                    ...
                    className="md:hidden border-t border-gray-100 bg-white/95 backdrop-blur-md px-4 pb-4 space-y-1 overflow-hidden"
                >
                    ...
                    {accessToken ? (
                        <>
                            ...
                            {unreadCount > 0 && <div className="py-2 text-sm text-emerald-600 font-medium"><FiBell /> {unreadCount} new notification{unreadCount > 1 ? 's' : ''}</div>}
                        ...
                        <>
                            ...
                            <Link to="/register" className="block py-2.5 text-emerald-600 font-semibold text-sm" onClick={() => setMenuOpen(false)}>Sign Up</Link>
                        </>
                    )}
                </motion.div>
            )}
