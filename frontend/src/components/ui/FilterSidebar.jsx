...
const Toggle = ({ label, value, onChange }) => (
 ...
 className={`relative w-11 h-6 rounded-full transition-colors duration-300 ease-in-out flex items-center p-1 ${value ? 'bg-emerald-500' : 'bg-gray-200'}`}
 >
...
export default function FilterSidebar({ filters, options, onChange, onReset, isOpen, onClose }) {
 ...
 <div className="fixed inset-0 bg-black/30 z-20 lg:hidden" onClick={onClose} />
    ...
    {/* Sidebar */}
    <aside className={`
        fixed lg:sticky lg:top-24 left-0 h-screen lg:h-[calc(100vh-8rem)] w-72 bg-white border-r lg:border border-gray-200
        lg:rounded-xl lg:shadow-sm overflow-y-auto z-30 transition-transform duration-300 custom-scrollbar
 ...
 {activeCount > 0 && (
 <span className="badge bg-emerald-500 text-white text-xs">{activeCount}</span>
 )}
 </div>
 <div className="flex items-center gap-2">
 {activeCount > 0 && (
