...
const TYPE_ICONS = {
 ...
 place_approved: <FiMap />,
 booking_timeout: <FiClock />,
...
export default function NotificationBell() {
 ...
 <button
 onClick={handleOpen}
 className="relative p-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-600 hover:text-emerald-600"
 >
 <FiBell size={20} />
 {unread > 0 && (
 ...
 <FiBell size={15} className="text-emerald-500" />
 ...
 <FiAlertTriangle size={24} className="mx-auto mb-2 text-amber-500" />
 <p className="text-sm">Could not load notifications</p>
 <button type="button" onClick={() => refetch()} className="text-xs text-emerald-600 hover:underline mt-2">
 Retry
 </button>
 </div>
 ...
 <p className="text-xs mt-1 text-gray-400">Booking updates will appear here</p>
 </div>
 ) : (
 notifications.map(n => (
 <button
