...
export default function OTPModal({ email, purpose = 'signup', onVerified, onClose }) {
 ...
 setLoading(true)
 try {
 const res = await authApi.verifyOtp({ email, otp_code: code, purpose })
 ...
 ${digit ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-300 focus:border-emerald-400'}`}
 />
 ))}
 </div>
 ...
 className="flex items-center gap-1.5 text-sm mx-auto text-gray-500 hover:text-emerald-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
 >
 <FiRefreshCw size={14} />
