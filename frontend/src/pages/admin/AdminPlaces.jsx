...
export default function AdminPlaces() {
 ...
 {isFetching && (
 <div className="absolute top-0 right-0 z-10 flex items-center gap-1.5 text-xs text-emerald-600 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full shadow-sm border border-emerald-100">
 ...
 onError={e => { e.target.style.display='none'; e.target.parentElement.classList.add('bg-gradient-to-br','from-emerald-50','to-gray-100') }}
 />
 : <div className="w-full h-full flex items-center justify-center text-3xl opacity-30"><FiMapPin /></div>
 }
 <div className="absolute top-2 right-2 flex gap-1">
 <button onClick={() => setModal(p)}
 className="w-7 h-7 bg-white rounded-full flex items-center justify-center shadow hover:bg-emerald-50 transition-colors">
 <FiEdit2 size={12} className="text-emerald-600" />
 </button>
