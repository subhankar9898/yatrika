...
function SlotCalendar({ guideId, vacationDates, onSlotSelect }) {
    ...
    // Always show a fixed 7-day window: today → today+6
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-4">
                <p className="text-sm font-semibold text-gray-700">
                    {format(today, 'MMM d')} – {format(addDays(today, 6), 'MMM d, yyyy')}
                </p>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${totalAvailable > 0
                    ? 'bg-emerald-50 text-emerald-700'
                    : 'bg-gray-100 text-gray-500'
                    }`}>
                    {isLoading ? 'Loading…' : totalAvailable > 0 ? `${totalAvailable} slot${totalAvailable > 1 ? 's' : ''} available` : 'No slots this week'}
                </span>
            </div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
                {days.map(day => {
                    ...
                    const isToday = dateStr === from

                    return (
                        <div
                            key={dateStr}
                            className={`rounded-xl p-1.5 min-h-[100px] border ${isToday
                        ...
                        >
                            <p className={`text-xs font-bold mb-1.5 text-center ${isToday ? 'text-emerald-700' : 'text-gray-500'
                                }`}>
                                {format(day, 'EEE')}
                                <br />
                                <span className={`text-sm ${isToday
                                    ? 'bg-emerald-500 text-white w-6 h-6 rounded-full inline-flex items-center justify-center'
                                    : 'text-gray-700'
                                    }`}>
                                    {format(day, 'd')}
                                </span>
                            ...
                            {isVacation ? (
                                <p className="text-xs text-center text-gray-400 mt-2"><FiSun size={12} className="inline" /> Off</p>
                            ) : daySlots.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                    {daySlots.map(slot => (
                                        <button
                                            ...
                                            className={`w-full text-xs px-1 py-1 rounded-lg border font-medium transition-all ${STATUS_STYLE[slot.status]
                                                }`}
                                            title={STATUS_LABEL[slot.status]}
                                        >
                                            {slot.start_time?.slice(0, 5)}<br />
                                            <span className="opacity-70 text-[10px]">{STATUS_LABEL[slot.status]}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-fade-in-up">
                ...
                <div className="bg-emerald-50 rounded-xl p-4 mb-4 space-y-2 text-sm">
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-500 hover:text-emerald-600 mb-6 transition-colors">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
                    <div className="flex items-start gap-5 flex-wrap">
                        <div className="w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-3xl flex-shrink-0 overflow-hidden">
                            <img 
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-2 text-sm text-gray-500 font-medium">
                                ...
                                <a href={`mailto:${guide.email}`} className="hover:text-emerald-600 transition-colors">
                                    {guide.email}
                                </a>
                            </div>
                            {guide.languages?.length > 0 && (
                                <div className="flex gap-1.5 mt-3 flex-wrap">
                                    ...
                                    {guide.languages.map(l => (
                                        <span key={l} className="badge bg-emerald-50 text-emerald-700 text-xs">{l}</span>
                                    ))}
                                </div>
                            )}
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {guide.places.map(p => (
                                    <span key={p.id} onClick={() => navigate(`/places/${p.id}`)}
                                        className="flex items-center gap-1.5 badge bg-gray-100 text-gray-600 text-xs cursor-pointer hover:bg-emerald-100 hover:text-emerald-700 transition-colors py-1 px-3">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
                    <h2 className="font-bold text-gray-800 text-lg mb-1 flex items-center gap-2">
                        <FiCalendar className="text-emerald-500" /> Availability
                    </h2>
